...
| Cap the content width                    | The General tab is a centred column at most `CONTENT_MAX_WIDTH` wide; a wider window adds margin, not longer rows. The dashboard has a fixed width.                                                                                                                                                  | `src/settings/widgets.rs`, `src/settings/general_tab.rs`    |
| One trailing control per row             | `Rows::row` takes exactly one `control`. A secondary action on the same setting sits in the subtitle as a small button (the pin's "Clear" in the Tray group).                                                                                                                                        | `src/settings/widgets.rs`, `src/settings/general_tab.rs`    |
...
| Same words on every surface              | The dashboard row, the tray menu row and the tray tooltip are built from `charge_value` and `status_note` (`device_line` joins them). Devices are ordered by `roster_order`: online first, then by name, ignoring case.                                                                              | `src/domain/text.rs`, `src/domain/roster.rs`                |
...
| Contrast is measured, not eyeballed      | `gui::contrast_ratio` (WCAG 2.1) backs tests: dashboard secondary text ≥ 4.5:1 on the panel, text on the accent ≥ 4.5:1 (`readable_on` picks black or white). The icon `Theme` colours are chosen for ≥ 3:1 as graphical objects, dimmed fill included.                                              | `src/gui/mod.rs`, `src/dashboard/mod.rs`, `src/icon/mod.rs` |
| Custom widgets are accessible widgets    | `switch`, `tile` and the tabs report a role and label through `widget_info` (checkbox, radio button, selectable label), take keyboard focus and draw a focus ring. Both windows export an AT-SPI tree through eframe's `accesskit` feature.                                                          | `src/settings/widgets.rs`, `Cargo.toml`                     |
...
are private constants in `src/dashboard/mod.rs`.

### Spacing and size

| Token                       | Value     | Use                                                                             |
| --------------------------- | --------- | ------------------------------------------------------------------------------- |
| `CONTENT_MAX_WIDTH` (`pub`) | 640       | Widest the General tab column gets                                              |
| `ROW_MIN_HEIGHT`            | 48        | Minimum row height in a group                                                   |
...
| `WINDOW_DEFAULT_SIZE`       | 980 × 620 | Settings window opening size (`src/settings/mod.rs`)                            |
| `WINDOW_MIN_SIZE`           | 940 × 360 | Settings window minimum, derived from the Devices table (`src/settings/mod.rs`) |
| `WINDOW_WIDTH`              | 380       | Dashboard width                                                                 |
| `MARGIN`                    | 12        | Dashboard panel margin (the settings panel margin is a literal 16)              |
| `ROW_HEIGHT`                | 48        | Dashboard row                                                                   |
| `ROW_PADDING`               | 5         | Dashboard row's vertical inset                                                  |
| `GLYPH_COLUMN`              | 30        | Dashboard kind-glyph column                                                     |
...
| Switch knob                 | Track radius − `KNOB_INSET` | 0.5, black at alpha 60                                                                                                     |
...
| Body text        | `visuals.text_color()`                                                        | Everything, and the dashboard's secondary text (`secondary_text`)                            |
| Strong text      | `visuals.strong_text_color()`                                                 | Selected tab, selected tile caption                                                          |
| Weak text        | `.weak()` / `visuals.weak_text_color()`                                       | `secondary`, idle tab                                                                        |
| Warning / error  | `visuals.warn_fg_color` / `visuals.error_fg_color`                            | Devices tab: unanswered tray; armed delete confirm                                           |
...
| Secondary                | Body × `SECONDARY_SCALE` (0.88), weak | `secondary`, `footer`     |
| Dashboard name and value | Body, `.strong()`                     | `render_row`              |
| Dashboard note           | `NOTE_SIZE` (12), `secondary_text`    | `render_row`              |
| Dashboard kind glyph     | `GLYPH_SIZE` (20)                     | `render_row`              |
...
| `secondary(ui, text)`                 | Weak text at `SECONDARY_SCALE`                                                                          | Hints and footers, including custom subtitles                  |
...
threshold), `ui.small_button` for a secondary action in a subtitle.
...
- **Defaults with per-device overrides** say so in the group footer (`defaults-hint`). Today an
  override is a checkbox labelled "Use default (…)" in the Devices tab
  (`render_threshold_override`, `render_interval_override`). There is no "↺" reset control yet;
  see [Planned](#planned-devices-tab-as-expander-rows-r86).

## Surfaces

### Settings window

`rigbat settings`, a separate process (`src/settings/`). A decorated window titled `rigbat`, a
`tab_bar` over two views (`Tab::General`, `Tab::Devices`), panel margin 16.
...
- **Devices** (`src/settings/devices_tab.rs`): a search field, a Refresh button, a sortable
  `egui_extras` table and a detail band for the selected device. It predates the components above;
  R86 restyles it.
- Esc cancels an armed delete, then clears the search, then closes the window (`escape_action`).
...
- Undecorated, `WINDOW_WIDTH` wide, exactly as tall as its rows plus footer (`window_size`), and it
...
- A surface test lists every string it expects, then loops over `Lang::ALL`: each must appear with
  ...
  `every_widget_paints_its_text_whole_on_one_line`.
- Test sizes are the real constraints: General at `GENERAL_TAB_TEST_SIZE` (the `WINDOW_MIN_SIZE`
  width), the widgets at `CONTENT_MAX_WIDTH`, the dashboard at its own `wanted_size()`, the
  Devices table in `device_table_fits_at_the_minimum_window_width`.
- Interaction: `run_frame` lays the page out, `ctx.read_response` finds a widget by its global id
  (`switch_id`), `click_at` or a key event drives it, and the test asserts the saved config and the
  announced `WidgetInfo` (`a_switch_toggles_from_the_keyboard_and_carries_its_label`).
- Contrast and glyphs have their own tests: `secondary_text_is_readable_on_the_panel_in_both_themes`,
...
## Planned: Devices tab as expander rows (R86)

Not implemented. The Devices tab moves from the table to the components above:

- Two `group`s: "Connected now" and "Seen before".
- A row: kind glyph, name, the dashboard's value (`charge_value`), and a show-in-tray `switch`.
- A row expands in place to "Show on the single icon", the threshold and interval overrides with a
  reset control, and "Remove…".
- Rows are ordered by the domain roster (`roster_order`), not by sortable columns.
- Search appears only when the list is long.
